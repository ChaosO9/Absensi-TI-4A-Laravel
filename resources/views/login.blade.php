...
<body class="login">
    <div class="container">
        ...
        <div class="row justify-content-center form-login mt-5">
            <div class="col-md-6">
                <form action="{{ route('login') }}" class="needs-validation panel" method="post" novalidate>
                    @csrf
                    <h3 class="mb-4 text-center text-uppercase">{{ __('Login Sistem Absensi TI 4A') }}</h3>
                    @if (Session::has('failed'))
                        <div class="alert alert-danger mr-5 ml-5 radius text-center" role="alert">
                            {{ __(Session::get('failed')) }}
                        </div>
                    @endif
                    <div class="form-group ml-5 mr-5">
                        <input type="text" name="email_or_nim" id="email"
                            class="form-control form-control-lg radius" placeholder="{{ __('NIM') }}/Email"
                            required>
                    ...
                    <div class="form-group ml-5 mr-5">
                        <input type="password" name="password" id="password"
                            class="form-control form-control-lg radius" placeholder="Password" required>
                        <div class="invalid-feedback">
                    ...
                    <div class="form-group mt-4 ml-5 mr-5 mb-3">
                        <a href="{{ route('registrasiadmin') }}" class="btn btn-info btn-regis block radius"
                            role="button">{{ __('Registrasi Admin') }}</a>
                    </div>
                    <div class="row">
                        <div class="col-auto">
